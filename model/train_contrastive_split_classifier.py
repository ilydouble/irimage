...
class ContrastiveThermalDataset(Dataset):
    def __init__(self, root_dir: str, transform: T.Compose, mode: str = 'train',
                 use_asymmetry_analysis: bool = False, indices: Optional[List[int]] = None):
        ...
        # 原始 class_to_images
        full_class_to_images = self._find_classes()

        # 如果提供了 indices，只保留对应的图片
        if indices is not None:
            # flatten images
            all_imgs = []
            img_to_class = {}
            for cls_name, imgs in full_class_to_images.items():
                for img in imgs:
                    all_imgs.append(img)
                    img_to_class[img] = cls_name
            # 根据 indices 筛选
            selected_imgs = [all_imgs[i] for i in indices]
            # 重建 class_to_images
            self.class_to_images = {}
            for img in selected_imgs:
                cls_name = img_to_class[img]
                self.class_to_images.setdefault(cls_name, []).append(img)
        else:
            self.class_to_images = full_class_to_images

        self.classes = list(self.class_to_images.keys())

        # 构建 image_paths 和 labels（用于 classification 模式）
        self.image_paths = []
        self.labels = []
        for class_idx, class_name in enumerate(self.classes):
            imgs = self.class_to_images[class_name]
    ...
    def _get_contrastive_pair(self, index: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        while attempts < 10:
            try:
                if index % 2 == 0:  # 正样本对
                    label = 1.0
                    # 从 class_to_images 中随机选一个类
                    class_name = random.choice(self.classes)
                    if len(self.class_to_images[class_name]) < 2:
                        attempts += 1
                        continue
                    img1, img2 = random.sample(self.class_to_images[class_name], 2)
                else:  # 负样本对
                    label = 0.0
                ...
                # 处理图像
                tensor1 = self._process_image(img1)
                tensor2 = self._process_image(img2)
    ...
    def _process_image(self, img_path: str) -> Optional[torch.Tensor]:
        try:
            ...
            if self.use_asymmetry_analysis:
                ...
                if halves is None:
                    return None
                left_tensor = self.transform(halves[0])
                right_tensor = self.transform(halves[1])
...
class ContrastiveThermalClassifier:
    ...
    def create_descriptive_output_dir(self, contrastive_epochs, classification_epochs, contrastive_lr, classification_lr, batch_size):
        ...
        # 格式: CONTRASTIVE-SPLIT-E50+30-LR0.001+0.0001-B32-ASYM-20240829_143022
        asymmetry_flag = "ASYM" if self.use_asymmetry_analysis else "FULL"
        folder_name = f"CONTRASTIVE-SPLIT-E{contrastive_epochs}+{classification_epochs}-LR{contrastive_lr}+{classification_lr}-B{batch_size}-{asymmetry_flag}-{timestamp}"
    ...
    def train_contrastive_encoder(self, epochs=50, batch_size=32, lr=0.001, dataset=None):
        """第一阶段：对比学习训练编码器"""
        ...
        criterion = RegularizedContrastiveLoss(margin=0.2, temperature=0.1) #调参数0.2->0.5
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (img1, img2, labels) in enumerate(dataloader):
                ...
                # 计算损失
                if isinstance(criterion, RegularizedContrastiveLoss):
                    # RegularizedContrastiveLoss 需要 model 参数
                    loss_result = criterion(feat1, feat2, labels, model)
                    loss, contrastive_loss, reg_loss = loss_result
                elif isinstance(criterion, AdaptiveContrastiveLoss):
                    # AdaptiveContrastiveLoss 也需要 model 参数
                    loss_result = criterion(feat1, feat2, labels, model)
                    loss, contrastive_loss, reg_loss = loss_result
                else:
                    # 其他损失函数只返回单个损失值
    ...
    def train_classifier(self, pretrained_encoder=None, epochs=30, batch_size=32, lr=0.0001, train_dataset=None, val_dataset=None, test_dataset=None, train_labels=None, val_labels=None, test_labels=None):
        """第二阶段：微调分类器 - 处理数据不平衡"""
        print("=== 第二阶段：微调分类器 ===")
    ...
    def run_full_training(self, contrastive_epochs=50, classification_epochs=30,
        ...
        full_dataset_cls = ContrastiveThermalDataset(self.data_dir, self.transform, mode='classification')
        all_labels = [full_dataset_cls[i][1] for i in range(len(full_dataset_cls))]

        from sklearn.model_selection import train_test_split
        indices = list(range(len(full_dataset_cls)))
        train_indices, temp_indices = train_test_split(indices, test_size=0.3, stratify=all_labels, random_state=42)
        ...
        combined_dataset = ContrastiveThermalDataset(
            self.data_dir, self.transform, mode='contrastive', use_asymmetry_analysis=self.use_asymmetry_analysis, indices=combined_indices)


        train_dataset_cls = ContrastiveThermalDataset(
            self.data_dir, self.transform, mode='classification', use_asymmetry_analysis=self.use_asymmetry_analysis, indices=train_indices
        )
        val_dataset_cls = ContrastiveThermalDataset(
            self.data_dir, self.transform, mode='classification', use_asymmetry_analysis=self.use_asymmetry_analysis, indices=val_indices
        )
        test_dataset_cls = ContrastiveThermalDataset(
            self.data_dir, self.transform, mode='classification', use_asymmetry_analysis=self.use_asymmetry_analysis, indices=test_indices
        )
        ...
        if skip_contrastive and self.pretrained_encoder_path:
            ...
            # 第一阶段：对比学习（只使用训练+验证集）
            print("=== 执行完整的两阶段训练 ===")
            encoder = self.train_contrastive_encoder(epochs=15, batch_size=32, lr=0.00005, dataset=combined_dataset)
        
        # 第二阶段：分类微调（使用训练+验证集训练，测试集评估）
        print("=== 第二阶段：分类微调 ===")
        model, test_results = self.train_classifier(encoder, epochs=30, batch_size=32, lr=0.00001, train_dataset=train_dataset_cls, val_dataset=val_dataset_cls, test_dataset=test_dataset_cls, train_labels=train_labels, val_labels=val_labels, test_labels=test_labels)
